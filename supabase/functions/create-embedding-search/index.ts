...
serve(async (req)=>{
  ...
  try {
    ...
    if (assetPlatformCombination) {
      console.log('Requested asset-platform combination:', `${assetPlatformCombination.assets} assets from ${assetPlatformCombination.platforms} platforms`);
    }
    ...
    switch(promptType){
      ...
      default:
        promptContent = `
          ...
          ${processedAssets.slice(0, 5).map((asset)=>`- ${asset.name} (${asset.platform_name}, ${asset.platform_industry}): Buy type: ${asset.buy_types}, Cost: ${asset.amount}, Est. impressions: ${asset.estimated_impressions}, Est. clicks: ${asset.estimated_clicks}`).join('\n')}
