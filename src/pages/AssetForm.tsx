...
import { PLACEMENT_OPTIONS, BUY_TYPE_OPTIONS, AD_FORMAT_OPTIONS, AD_TYPE_OPTIONS, CATEGORY_OPTIONS, DELIVERABLES_OPTIONS, CTA_OPTIONS } from "@/types/asset";
interface Platform {
  id: string;
  name: string;
}
  ...
  const fetchPlatforms = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("platforms").select("id, name");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit && id) {
        ...
        result = await supabase.from("assets").update(assetData).eq("id", id);
        if (result.error) throw result.error;
        assetId = id;
      } else {
        console.log('Creating new asset');
        result = await supabase.from("assets").insert([{
          ...assetData,
      ...
      <div className="animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="name" name="name" placeholder="Enter asset name" value={formData.name} onChange={handleInputChange} className="mt-1.5 bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" required />
                </div>
                
                <div>
                  <Label htmlFor="platform">Platform*</Label>
                  <Select value={formData.platform_id} onValueChange={value => handleSelectChange("platform_id", value)} required>
                ...
                <div>
                  ...
                  <Select value={formData.ad_format} onValueChange={value => handleSelectChange("ad_format", value)}>
                    <SelectTrigger className="mt-1.5 bg-white border-none neu-flat hover:shadow-neu-pressed">
                      <SelectValue placeholder="Select ad format" />
                    </SelectTrigger>
                    <SelectContent>
                      {AD_FORMAT_OPTIONS.map(format => <SelectItem key={format} value={format}>
                          {format}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div>
                  ...
                  <Select value={formData.ad_type} onValueChange={value => handleSelectChange("ad_type", value)}>
                    <SelectTrigger className="mt-1.5 bg-white border-none neu-flat hover:shadow-neu-pressed">
                      <SelectValue placeholder="Select ad type" />
                    </SelectTrigger>
                    <SelectContent>
                      {AD_TYPE_OPTIONS.map(type => <SelectItem key={type} value={type}>
                          {type}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div>
                  ...
                  <Select value={formData.category} onValueChange={value => handleSelectChange("category", value)}>
                    ...
                    <SelectContent>
                      {CATEGORY_OPTIONS.map(category => <SelectItem key={category} value={category}>
                          {category}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div>
                  ...
                  <Select value={formData.placement} onValueChange={value => handleSelectChange("placement", value)}>
                    ...
                    <SelectContent>
                      {PLACEMENT_OPTIONS.map(placement => <SelectItem key={placement} value={placement}>
                          {placement}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Textarea id="description" name="description" placeholder="Enter asset description" value={formData.description} onChange={handleInputChange} className="mt-1.5 min-h-[120px] bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" />
                </div>
                
                <div>
                  <Label htmlFor="tags">Tags</Label>
                  <div className="flex mt-1.5">
                    <Input id="tagInput" name="tagInput" placeholder="Add tags" value={formData.tagInput} onChange={handleInputChange} onKeyDown={handleKeyDown} className="flex-1 bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" />
                    <NeuButton type="button" onClick={handleAddTag} className="ml-2">
                      Add
                    </NeuButton>
                  </div>
                  
                  <div className="flex flex-wrap gap-2 mt-2">
                    {formData.tags.map((tag, index) => <span key={index} className="inline-flex items-center bg-neugray-200 rounded-full px-2 py-1 text-sm">
                        {tag}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  ...
                  <Select value={formData.deliverables} onValueChange={value => handleSelectChange("deliverables", value)}>
                    <SelectTrigger className="mt-1.5 bg-white border-none neu-flat hover:shadow-neu-pressed">
                      <SelectValue placeholder="Select deliverables" />
                    </SelectTrigger>
                    <SelectContent>
                      {DELIVERABLES_OPTIONS.map(deliverable => <SelectItem key={deliverable} value={deliverable}>
                          {deliverable}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Switch id="has_cta" checked={formData.has_cta} onCheckedChange={handleCtaToggleChange} />
                ...
                <div>
                  ...
                  <Input id="snapshot_ref" name="snapshot_ref" placeholder="Enter reference number" value={formData.snapshot_ref} onChange={handleInputChange} className="mt-1.5 bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" />
                </div>
              </div>
            </div>
          </NeuCard>
          ...
          <NeuCard>
            ...
            <BuyTypeSelector buyType={formData.buy_types} amount={formData.amount} onChange={handleFieldChange} />
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
              <div>
                <Label htmlFor="minimum_cost">Minimum Cost</Label>
                <Input id="minimum_cost" name="minimum_cost" type="number" placeholder="Enter minimum cost" value={formData.minimum_cost} onChange={handleNumberInput} className="mt-1.5 bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" />
              </div>
              
              <div>
                <Label htmlFor="moq">MOQ (Min Order Quantity)</Label>
                <Input id="moq" name="moq" placeholder="Enter MOQ" value={formData.moq} onChange={handleInputChange} className="mt-1.5 bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" />
              </div>

              <div>
                <Label htmlFor="rate_inr">Rate in INR</Label>
                <Input id="rate_inr" name="rate_inr" type="number" placeholder="Enter rate in INR" value={formData.rate_inr} onChange={handleNumberInput} className="mt-1.5 bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" />
              </div>
              
              <div>
                <Label htmlFor="gtm_rate">GTM Rate</Label>
                <Input id="gtm_rate" name="gtm_rate" type="number" placeholder="Enter GTM rate" value={formData.gtm_rate} onChange={handleNumberInput} className="mt-1.5 bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" />
              </div>
            </div>
          </NeuCard>
          ...
          <NeuCard>
            ...
            <div>
              ...
              <input type="file" accept="image/*" ref={thumbnailInputRef} onChange={e => handleFileChange(e, 'thumbnail')} className="hidden" />
              
              <div className={`mt-1.5 border-2 border-dashed rounded-lg p-6 text-center ${files.thumbnailPreview ? 'border-primary' : 'border-gray-300'}`} onDrop={e => handleFileDrop(e, 'thumbnail')} onDragOver={handleDragOver}>
                {files.thumbnailPreview ? <div className="space-y-2">
                    <div className="flex items-center justify-center">
                      <div className="w-full h-40 overflow-hidden flex items-center justify-center bg-neugray-200 rounded-lg">
                        <img src={files.thumbnailPreview} alt="Thumbnail preview" className="max-w-full max-h-40 object-contain" />
                      </div>
                    </div>
                    <div className="flex justify-end">
                      <button type="button" onClick={() => removeFile('thumbnail')} className="text-red-500 hover:text-red-700 text-sm">
                        Remove
                      </button>
                    </div>
                  </div> : <div>
                    ...
                    <NeuButton type="button" variant="outline" size="sm" onClick={() => handleBrowseClick('thumbnail')}>
                      Browse Images
                    </NeuButton>
                  </div>}
              </div>
          ...
          <NeuCard>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <Input id="ctr" name="ctr" type="number" step="0.01" min="0" max="100" placeholder="Enter CTR percentage" value={formData.ctr} onChange={e => handleFieldChange('ctr', parseFloat(e.target.value) || 0)} className="mt-1.5 bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" />
              </div>

              <div>
                <Label htmlFor="vtr">VTR (%)</Label>
                <Input id="vtr" name="vtr" type="number" step="0.01" min="0" max="100" placeholder="Enter VTR percentage" value={formData.vtr} onChange={e => handleFieldChange('vtr', parseFloat(e.target.value) || 0)} className="mt-1.5 bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" />
              </div>
            </div>
          </NeuCard>
