  ...
  return <Layout>
      <div className="animate-fade-in">
        ...
        <NeuCard className="mb-8">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search platforms..." className="pl-10 w-full bg-white border-none neu-pressed focus-visible:ring-0 focus-visible:ring-offset-0" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
            <div className="flex gap-2">
              <NeuButton variant="outline" className="flex items-center gap-1" onClick={() => setIndustryFilter("All")}>
                <Filter size={16} />
                Filter
              </NeuButton>
          ...
          <div className="mt-4 flex gap-2 flex-wrap">
            ...
            {industries.map(industry => <span key={industry} className={`text-xs py-1 px-2 neu-flat hover:shadow-neu-pressed cursor-pointer ${industryFilter === industry ? 'shadow-neu-pressed' : ''}`} onClick={() => setIndustryFilter(industry)}>
                {industry}
              </span>)}
          </div>
        </NeuCard>
        ...
        {loading ? <div className="flex justify-center items-center py-20">
          ...
          </div> : filteredPlatforms.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {filteredPlatforms.map(platform => <Link key={platform.id} to={`/platforms/${platform.id}`}>
                <NeuCard className="h-full hover:shadow-neu-pressed transition-all cursor-pointer animate-scale-in">
                  ...
                  width: `${platform.device_split?.ios || 50}%`
                }}></div>
                    </div>
          ...
          </div> : <NeuCard className="py-10 text-center mb-8">
